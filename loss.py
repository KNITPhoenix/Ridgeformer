...
class DualMSLoss_FineGrained(nn.Module):
    ...
    def ms_sample_cbcb_clcl(self,sim_mat,label):
        ...
        # pos_mask = torch.transpose(pos_mask[mask_pos].reshape(n_sha, n_sha-1),0,1)
    ...
    def forward(self, x_contactless, x_contactbased, x_cl_tokens, x_cb_tokens, labels, device):
        ...
        # mask = mask.triu(1) | mask.tril(-1)
        # sim_mat_clcl = torch.transpose(sim_mat_clcl[mask].reshape(n, n-1),0,1)

        sim_mat_cbcb = F.linear(self.l2_norm(x_contactbased), self.l2_norm(x_contactbased))
        # n = sim_mat_cbcb.shape[0]
        # mask = torch.ones(n, n, dtype=torch.bool)
        # mask = mask.triu(1) | mask.tril(-1)
        # sim_mat_cbcb = torch.transpose(sim_mat_cbcb[mask].reshape(n, n-1),0,1)
        
        sim_mat_cbcl = F.linear(self.l2_norm(x_contactbased), self.l2_norm(x_contactless))

        # fin_sim_mat_clcb   = self.compute_sharded_cosine_similarity(x_cb_tokens, x_cl_tokens, 10)
        ...
        # cb_cl_weighted = sim_mat_cbcl + fin_sim_mat_clcb
        ...
        # loss5              = self.ms_sample(cb_cl_weighted, labels).cuda() + self.ms_sample(cb_cl_weighted.t(), labels).cuda()
        print(loss4)
        print(loss3)
        print(loss2)
...
class DualMSLoss_FineGrained_domain_agnostic(nn.Module):
    ...
    def forward(self, x_contactless, x_contactbased, x_cl_tokens, x_cb_tokens, labels, device, domain_class_cl, domain_class_cb, domain_class_cl_gt, domain_class_cb_gt):
        ...
        # print(domain_class_cl)
        # print(domain_class_cl_gt)
        ...
        # print(loss4, loss2, loss3, domain_class_loss)

        # return loss4  + loss2 + loss3 + (30*domain_class_loss)  #+ loss2 + loss3 #+ (1.5*loss2) + (1.5*loss3)  # + loss2 + loss3#+ loss5 # 0.1*loss5  + loss3
